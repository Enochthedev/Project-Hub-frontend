...
import { MessageCircle, TrendingUp, Star, Clock, Bookmark, AlertTriangle, CheckCircle, Zap } from "lucide-react"
...
export function ConversationAnalytics({ className }: ConversationAnalyticsProps) {
  ...
  const calculateAnalytics = () => {
    ...
    const filteredConversations = conversations.filter((conv) => new Date(conv.createdAt) >= cutoffDate)
    ...
    const escalationRate =
      filteredConversations.length > 0 ? (escalatedConversations.length / filteredConversations.length) * 100 : 0
  ...
  return (
    <div className={cn("space-y-6", className)}>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-[#534D56] dark:text-[#F8F1FF]">Conversation Analytics</h2>
        <div className="flex items-center gap-2">
          {(["7d", "30d", "90d"] as const).map((range) => (
            <Badge
              key={range}
              variant={timeRange === range ? "default" : "outline"}
              className={cn("cursor-pointer", timeRange === range && "bg-[#1B998B] text-white")}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-[#656176] dark:text-[#DECDF5]">{analyticsData.totalMessages} total messages</p>
          </CardContent>
        </Card>
            ...
            <div className="flex items-center mt-1">
              {[1, 2, 3, 4, 5].map((star) => (
                <Star
                  ...
                  className={cn(
                    "h-3 w-3",
                    star <= analyticsData.averageRating ? "fill-yellow-400 text-yellow-400" : "text-gray-300",
                  )}
                />
              ))}
            </div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-[#656176] dark:text-[#DECDF5]">Average response time</p>
          </CardContent>
        </Card>
              ...
              {analyticsData.confidenceScores.map((item, index) => (
                <div key={index} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-[#534D56] dark:text-[#F8F1FF]">{item.range}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Progress
                      ...
                      className="w-20 h-2"
                    />
                    <span className="text-sm text-[#656176] dark:text-[#DECDF5] w-8">{item.count}</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-[#656176] dark:text-[#DECDF5]">Helpful responses saved</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-[#656176] dark:text-[#DECDF5]">Conversations escalated</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-[#656176] dark:text-[#DECDF5]">Messages per conversation</p>
          </CardContent>
        </Card>
      </div>
    </div>
